...
<div class="form-builder__content grid-12">
            ...
            <mc-card class="col-12">
                <template #default>
                    <div class="input-group">
                        ...
                        <h6 class="input-group__subtitle">
                            <?= i::__("Permite que o inscrito dê nome a um projeto no momento da inscrição.") ?>
                        </h6>
                        <div class="input-group__inputs no-padding-bottom">
                            <label class="input-group__input"> <input v-model="entity.projectName"
                                    :checked="!entity.projectName || entity.projectName == '0'" type="radio"
                                    name="projectName" value="0" /> <?= i::_e('Desabilitado') ?> </label>
                            <label class="input-group__input"> <input v-model="entity.projectName" type="radio"
                                    name="projectName" value="2" /> <?= i::_e('Obrigatório') ?> </label>
                            <label class="input-group__input"> <input v-model="entity.projectName" type="radio"
                                    name="projectName" value="1" /> <?= i::_e('Opcional') ?> </label>
                        </div>
                    </div>
                </template>
            </mc-card>
            ...
            <mc-card class="col-12">
                <template #default>
                    <div class="input-group">
                        ...
                        <h6 class="input-group__subtitle">
                            <?= i::__("Solicita ao usuário que insira a imagem de perfil no formulário de inscrição") ?>
                        </h6>
                        <div class="input-group__inputs">
                            <entity-field class="input-box" :entity="entity" hide-required :editable="true"
                                prop="requestAgentAvatar" :autosave="3000"></entity-field>
                        </div>
                    </div>
                </template>
            </mc-card>
            ...
            <mc-card class="col-12">
                <template #default>
                    <div class="input-group">
                        ...
                        <div class="input-group__inputs">
                            <entity-field class="input-box" :entity="entity" hide-required :editable="true"
                                prop="enableQuotasQuestion" :autosave="3000"></entity-field>
                        </div>
                    </div>
                </template>
            </mc-card>
    ...
    <div class="col-12">
        <mc-tabs ref="tabs" v-model:draggable="stepsWithSlugs">
            <template #default>
                <mc-tab v-for="({ step, slug }, index) of stepsWithSlugs" :label="`${index + 1}. ${step.name ?? ''}`"
                    :key="step.id" :slug="slug" :cache="false">
                    <div class="form-builder__step-config">
                        ...
                        <div>
                            <opportunity-filter-configuration v-model="step.metadata.conditional"
                                @update:modelValue="saveMetadata(step)"></opportunity-filter-configuration>
                        </div>
                    </div>

                    <v1-embed-tool route="formbuilder" :id="entity.id" :params="{ step_id: step.id }"
                        min-height="600px"></v1-embed-tool>
                </mc-tab>
            </template>
            ...
            <template #after-tablist>
                <mc-modal teleport="body" title="<?php i::_e('Criar etapa') ?>">
                    <template #button="modal">
                        <button type="button" class="button button--primary button--icon form-builder__add-step"
                            @click="modal.open()">
                    ...
                    <template #actions="modal">
                        <button type="button" class="button button--primary"
                            @click="addStep(modal)"><?php i::_e('Criar') ?></button>
                    </template>
                </mc-modal>
            </template>
        </mc-tabs>
