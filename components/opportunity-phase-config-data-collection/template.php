...
<div class="opportunity-data-collection grid-12">
    ...
    <div class="opportunity-data-collection__limits col-12" v-if="phase.isFirstPhase">
        <div class="opportunity-data-collection__fields">
            <entity-field :entity="phase" prop="vacancies" :min="0" :autosave="3000" :hide-required="phase.isModel == 1" class="field__limits">
                <template #info>
                    <?php $this->info('editais-oportunidades -> configuracoes -> total-vagas') ?>
                </template>
            </entity-field>
            <entity-field :entity="phase" prop="totalResource" :min="0" :autosave="3000" :hide-required="phase.isModel == 1" class="field__limits">
                <template #info>
                    <?php $this->info('editais-oportunidades -> configuracoes -> valor-total') ?>
                </template>
            </entity-field>
            <!-- <entity-field :entity="phase" prop="registrationLimit" :min="0"  class="field__limits">
                        <template #info>
                            <?php $this->info('editais-oportunidades -> configuracoes -> limite-inscricoes') ?>
                        </template>
                    </entity-field> -->
            <!-- <entity-field :entity="phase" prop="registrationLimitPerOwner" :min="0"  class="field__limits">
                        <template #info>
    ...
    <div class="col-12" v-if="phase.isFirstPhase">
        ...
        <opportunity-reserva-vagas-cotas :entity="phase" class="col-12"></opportunity-reserva-vagas-cotas>
    </div>
    ...
    <div class="col-12 grid-12 opportunity-data-collection__config-button">
        ...
        <mc-link :entity="phase" route='formBuilder' class="config-phase__info-button button--primary button col-6"
            icon="external" right-icon>
            <?= i::__("Configurar formulário") ?>
        </mc-link>
        <?php $this->applyTemplateHook('opportunity-data-collection-config-form', 'end') ?>
    </div>
    <?php $this->applyTemplateHook('opportunity-data-collection-config-form', 'after') ?>
    ...
    <template v-if="nextPhase?.__objectType != 'evaluationmethodconfiguration'">
        ...
        <opportunity-phase-publish-date-config :phase="phase" :phases="phases" hide-description hide-button
            useSealsCertification></opportunity-phase-publish-date-config>
    </template>
