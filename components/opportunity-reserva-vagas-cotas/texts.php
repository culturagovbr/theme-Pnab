<?php
use MapasCulturais\i;

return [
    'labelCota1' => i::__('Pessoas negras (pretas e pardas)'),
    'labelCota2' => i::__('Pessoas indígenas'),
    'labelCota3' => i::__('Pessoas com deficiência'),
    'sectionTitle' => i::__('Reserva de vagas (cotas)'),
    'sectionDescription' => i::__('Configure as cotas abaixo conforme a IN-MinC-10/2023. Marque "Não aplicável" quando a cota não se aplicar ao edital.'),
    'obrigatorio' => i::__('obrigatório'),
    'descricao' => i::__('Descrição'),
    'numeroVagas' => i::__('Número de vagas'),
    'valorDestinado' => i::__('Valor destinado'),
    'automatico' => i::__('Automático'),
    'naoAplicavel' => i::__('Não aplicável'),
    'infoBlockTitle' => i::__('Percentuais mínimos exigidos (IN-MinC-10/2023):'),
    'infoBlockItem1' => i::__('25% para Pessoas negras (pretas e pardas)'),
    'infoBlockItem2' => i::__('10% para Pessoas indígenas'),
    'infoBlockItem3' => i::__('5% para Pessoas com deficiência'),
    'validationError' => i::__('Configure todas as cotas ou marque como Não aplicável.'),
];
